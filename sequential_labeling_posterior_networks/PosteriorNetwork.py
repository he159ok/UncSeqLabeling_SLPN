...
class PosteriorNetwork(nn.Module):
    ...
    def forward(self, input, soft_output, return_output='hard', compute_loss=True):
        ...
        # print(zk.device)
        # print(input.device)
        # if zk.device != input.device:
        #     zk = zk.to(input.device)
        # above added by me
        ...
        if self.no_density:  # Ablated model without density estimation
            ...
            print(f'zk shape is {zk.shape}')
            print(f'log_q_zk shape is {log_q_zk.shape}')
            ...
            if isinstance(self.density_estimation, nn.ModuleList):
                for c in range(self.output_dim):
                    ...
                    print(f'log_p shape is {log_p.shape}')
                    log_q_zk[:, c] = log_p
        ...
        if compute_loss:
            if self.loss == 'CE':
                ...
                print(f'alpha shape in UCE loss is {alpha.shape}')
                print(f'soft_output shape in UCE loss is {soft_output.shape}')
                print(f'self.grad_loss shape in UCE loss is {self.grad_loss.shape}')
            else:
                raise NotImplementedError
