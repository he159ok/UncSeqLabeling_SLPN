...
class ModelTrainer:
        ...
        try:
            ...
            for epoch in range(epoch + 1, max_epochs + 1):
                ...
                for batch_no, batch in enumerate(batch_loader):
                    ...
                    # add use_small for one batch at here
                    if use_small == 1:
                        batch_steps = batch_steps[0:2]
                    ...
                    for batch_step in batch_steps:
                        ...
                        # print('forward_loss epoch is: ', epoch)
                        # try:
                        #     loss, datapoint_count = self.model.forward_loss(batch_step, epoch) # used for ENEN only
                        # except:
                        #     loss, datapoint_count = self.model.forward_loss(batch_step)

                        # in the model training process, use partial_pretrain parameter for the partial training.

                        ### add by me, which is a no-more-used situation
                        # try:
                        #     if self.model.unc_args.use_seq_training:
                        #         # used for sequential training
                        #         loss, datapoint_count = self.model.forward_loss(batch_step, epoch)
                        #     else:
                        #         loss, datapoint_count = self.model.forward_loss(batch_step)
                        # except:
                        #     # original
                ...
                if (
                    ...
                    log.info(f"update best model with val loss as {best_validation_score}")
    ...
    def final_test(
        ...
        test_dropout_num = 1,
        te_task=None,
        leave_out_labels=None,
        cal_unique_predict_scores=False,
        use_var_metric=False,
        ood_eval_mode ='entity_ori',
    ):
        base_path = Path(base_path)
        base_path.mkdir(exist_ok=True, parents=True)
        ...
        assert self.corpus.test
        if te_task is None or te_task == 'mis':
            test_results = self.model.evaluate(
                ...
                test_dropout_num=test_dropout_num,
                cal_unique_predict_scores=cal_unique_predict_scores,
            )
        elif te_task == 'ood':
            if ood_eval_mode == 'entity_ori':
                test_results = self.model.ood_evaluate(
                    self.corpus.test,
                    gold_label_type=self.model.label_type,
                    mini_batch_size=eval_mini_batch_size,
                    num_workers=num_workers,
                    ...
                    uncertainty_metrics=uncertainty_metrics,
                    test_dropout_num=test_dropout_num,
                    leave_out_labels=leave_out_labels,
                    cal_unique_predict_scores=cal_unique_predict_scores,
                    use_var_metric=use_var_metric,
                )
            elif ood_eval_mode == 'entity_fp':
                test_results = self.model.ood_evaluate_falsepos(
                    self.corpus.test,
                    gold_label_type=self.model.label_type,
                    mini_batch_size=eval_mini_batch_size,
                    num_workers=num_workers,
            ...
            elif ood_eval_mode == 'token_ori':
                test_results = self.model.ood_evaluate_token(
                    self.corpus.test,
                    gold_label_type=self.model.label_type,
                    mini_batch_size=eval_mini_batch_size,
                    num_workers=num_workers,
                ...
                raise ValueError(f'The ood_eval_mode={ood_eval_mode} is an invalid setting.')
        else:
            raise ValueError(f'The te_test={te_task} is an invalid setting.')

        if show_classification_res:
    ...
    def draw_tSNE(self,
        ...
        if use_data == 'tr':
            draw_data = self.corpus.train
        elif use_data == 'val':
            draw_data = self.corpus.dev
        elif use_data == 'te':
            draw_data = self.corpus.test
        else:
            raise ValueError(f'the use_data={use_data} is worng! [tr, val, te]')
