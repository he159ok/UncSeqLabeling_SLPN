...
class SequenceTagger_Dropout(flair.nn.EnsembleUncClassifier[Sentence]):
    def __init__(
        self,
        ...
        """
        super(SequenceTagger_Dropout, self).__init__()
    ...
    def forward_loss(self, sentences: List[Sentence]) -> Tuple[torch.Tensor, int]:
        ...
        sentence_tensor, lengths = self._prepare_tensors(sentences) # include embedding operation
            ...
            for batch in dataloader:
                ...
                for (sentence, sent_all_tags) in zip(batch, all_tags):
                    for (token, token_all_tags) in zip(sentence.tokens, sent_all_tags):
                        ...
                        token.append_tags_proba_dist('ensemble_predicted', token_all_tags)
                        # might need to add a new method to accumulate calculation
    ...
    def _standard_inference(self, features: torch.Tensor, batch: List[Sentence], probabilities_for_all_classes: bool, use_alpha: bool, sfmx_mode: str):
        ...
        if self.unc_args.unc_method == 'dp':
            if sfmx_mode == 'sfmx':
            ...
            raise ValueError('self.unc_args.unc_method != dp')
...
class MultiTagger:
    def __init__(self, name_to_tagger: Dict[str, SequenceTagger_Dropout]):
        super().__init__()
