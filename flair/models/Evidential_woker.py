...
def get_tagger_one_hot(label, N, off_value, on_value, input_mask, gpu):
    ...
    # label_mask = torch.unsqueeze(torch.where(label >= 0, 1, 0), dim=2) # in ENER, the 'O' is -100
    label_mask = torch.unsqueeze(torch.where(label > 0, 1, 0), dim=2) # in our setting, the 'O' label is 0
...
class Tagger_Evidence(nn.Module):
    ...
    def __init__(self, args):
        super().__init__()
        self.num_classes = args.output_dim
        self.etrans_func = 'exp'
        self.loss_type = 'edl'
        self.annealing_start = 0.01
        self.annealing_step = 10
        self.use_span_weight = False
        self.loss_func = CrossEntropyLoss()
        self.total_epoch = args.num_epochs # args.num_epochs # to do
        self.eps = 1e-10
        self.with_kldiv = True # whether use KL loss
        self.with_iw = True # whether use importance weight
        self.with_uc = True # whether use the
        self.gpu = True
    ...
    def ce_loss(self, sequence_logits, sequence_labels, input_mask, epoch):
        if input_mask is not None:
            ...
            active_labels = torch.where(
                active_loss, sequence_labels.view(-1),
                torch.tensor(self.loss_func.ignore_index).type_as(sequence_labels)
            )
    ...
    def edl_loss(self, logits, sequence_labels, input_mask, epoch):
        ...
        alp = evidence * (1 - soft_output) + 1  # items of evidence to be punished # alpha-wave
        beta = torch.ones_like(alp)  # Uniform Distribution
        ...
        # loss = (A + B - C)
        loss = torch.sum(soft_output * (torch.digamma(S) - torch.digamma(alpha)), dim=2, keepdim=False)
