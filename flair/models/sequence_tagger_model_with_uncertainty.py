...
from sequential_labeling_posterior_networks.SeqLabelingPosteriorNetwork import SeqLabelingPosteriorNetwork
from sequential_labeling_posterior_networks.TokenLevelPosteriorNetwork import TokenLevelPosteriorNetwork

from architectures.linear_sequential import linear_sequential

from gpn.layers import TokenLevelAlpha
from gpn.nn import loss


log = logging.getLogger("flair")


class SequenceTaggerWiUnc(flair.nn.UncClassifier[Sentence]):
    def __init__(
        self,
        ...
        """
        super(SequenceTaggerWiUnc, self).__init__()

        if unc_args.use_uce == 3:
            word_dropout = 0
            locked_dropout = 0
        ...
        log.info(f"SequenceTaggerWiUnc predicts: {self.label_dictionary}")
        ...
        # use the uncertainty related modelgu
        self.unc_args = unc_args
        use_eval = False
        if isinstance(self.unc_args.input_dims, str):
            use_eval = True
        if use_eval == True:
            self.unc_args.input_dims = eval(self.unc_args.input_dims)
            self.unc_args.hidden_dims = eval(self.unc_args.hidden_dims)
            if self.unc_args.kernel_dim != None:
                self.unc_args.kernel_dim = eval(self.unc_args.kernel_dim)
        else:
            self.unc_args.input_dims = self.unc_args.input_dims
            self.unc_args.hidden_dims = self.unc_args.hidden_dims
            if self.unc_args.kernel_dim != None:
                self.unc_args.kernel_dim = self.unc_args.kernel_dim

        # define a latent embedding encoder
        self.latent_enc = linear_sequential(input_dims=self.unc_args.input_dims,
                                            hidden_dims=self.unc_args.hidden_dims,
                                            output_dim=self.unc_args.latent_dim,
                                            k_lipschitz=self.unc_args.k_lipschitz)

        # related to sequential training and multi-task training
        if self.unc_args.use_seq_training==True or self.unc_args.use_multitask_training==True:
            self.latent2label_linear = torch.nn.Linear(self.unc_args.latent_dim, len(self.label_dictionary))


        self.token_level_pn = TokenLevelAlpha(
            dim_hidden=self.unc_args.input_dims,
            dim_latent=self.unc_args.latent_dim,
            num_classes=self.unc_args.output_dim,
            radial_layers=self.unc_args.radial_layers,
            maf_layers=self.unc_args.maf_layers,
            gaussian_layers=self.unc_args.gaussian_layers,
            use_batched_flow=True,
            alpha_evidence_scale='latent-new',  # ['latent-old', 'latent-new', 'latent-new-plus-classes', None]
            neighbor_mode=self.unc_args.neighbor_mode,
            normalize_dis=self.unc_args.normalize_dis,
            self_att_dk_ratio=self.unc_args.self_att_dk_ratio,
            self_att_droput=self.unc_args.self_att_droput,
            use_seq_training=self.unc_args.use_seq_training, # added for seq_tr
            hidden_dims = self.unc_args.hidden_dims, # added for seq_tr
            k_lipschitz=self.unc_args.k_lipschitz,
            use_stable=self.unc_args.use_stable,
        )
        ...
        if use_crf:
            if self.unc_args.unc_method in ['slpn']:
                raise ValueError('The unc in ViterbiLoss is not implemented yet!')
            elif self.unc_args.unc_method is None:
                self.loss_function = ViterbiLoss(self.label_dictionary)

        else:
            if self.unc_args.use_uce == None:
                if self.unc_args.unc_method in ['slpn']:
                    # self.loss_function = loss.uce_loss
                    self.loss_function = loss.uce_loss_and_reg
                elif self.unc_args.unc_method is None:
                    self.loss_function = torch.nn.CrossEntropyLoss(weight=self.loss_weights, reduction="sum")
            elif self.unc_args.use_uce == 0:
                self.loss_function = torch.nn.CrossEntropyLoss(weight=self.loss_weights, reduction="sum")
            elif self.unc_args.use_uce == 1:
                self.loss_function = loss.uce_loss_and_reg
            else:
                raise ValueError('use_uce is set wrongly')
    ...
    def forward_loss(self, sentences: List[Sentence],
        ...
        sentence_tensor, lengths = self._prepare_tensors(sentences) # include embedding operation sentence_tensor -> [bs, seq_len, token_emb_dim]

        # forward pass to get scores
        if self.unc_args.unc_method is None or self.unc_args.prior_mode == 'global':
            scores, latent_tensor = self.forward(sentence_tensor, lengths)
        elif self.unc_args.unc_method is not None and 'local' in self.unc_args.prior_mode:
            scores, latent_tensor = self.forward(sentence_tensor, lengths, sentences, self.unc_args.prior_mode)
        else:
            ValueError('the setting of unc_method and prior_mode is conflicted!')

        if self.unc_args.use_seq_training and not self.unc_args.finished_pretr_process:
            latent_scores = self.latent2label_linear(latent_tensor)
            latent_scores = latent_scores.squeeze(0)
            return torch.nn.CrossEntropyLoss(weight=self.loss_weights, reduction="sum")(latent_scores, gold_labels), len(gold_labels)



        if self.unc_args.use_multitask_training:
            latent_scores = self.latent2label_linear(latent_tensor)
            latent_scores = latent_scores.squeeze(0)
            # print(f'use_multitask_training is successful!')
            return self._calculate_loss(scores, gold_labels)[0] + self.unc_args.bert_loss_w * torch.nn.CrossEntropyLoss(weight=self.loss_weights, reduction="sum")(latent_scores, gold_labels), len(gold_labels)



        return self._calculate_loss(scores, gold_labels)

    def forward_loss_copy_before_multitask_sequential_training(self, sentences: List[Sentence]) -> Tuple[torch.Tensor, int]:
        ...
        if self.unc_args.unc_method is None or self.unc_args.prior_mode == 'global':
            ...
            scores = self.forward(sentence_tensor, lengths, sentences, self.unc_args.prior_mode)
        else:
            ValueError('the setting of unc_method and prior_mode is conflicted!')
    ...
    def forward(self, sentence_tensor: torch.Tensor, lengths: torch.LongTensor, sentences=None, prior_mode='global'):  # type: ignore[override]
        """
        ...
        if self.use_rnn:
            ...
            if self.use_dropout: # I believe this should be move to the use_rnn block
                sentence_tensor = self.dropout(sentence_tensor)
            if self.use_locked_dropout:
                sentence_tensor = self.locked_dropout(sentence_tensor)

        sentence_tensor = sentence_tensor.float()
        # cal culate token-level latent embedding (from uncertainty view using density-based method)
        if self.unc_args.unc_method in ['slpn']:


            emission_alpha, emission_beta, latent_tensor = self.token_level_pn(sentence_tensor, sentences, prior_mode, lengths, self.N_gl_cat_list, self.N_token_cat_list_dict, self.P_gl_cat_list, self.P_token_cat_list)


        # (to do) using token-level cat distribution (N_token_cat_list_dict)
        ...
        if self.unc_args.unc_method is None:
            if self.use_crf:
        ...
        elif self.unc_args.unc_method in ['slpn']:
            ## current implemntation is only for PN.
            if self.use_crf:
                emission_alpha = self.crf(emission_alpha)
                scores = (emission_alpha, lengths, self.crf.transitions)
            else:
                scores = self._get_scores_from_features(emission_alpha, lengths)


        return scores, latent_tensor

    def forward_copy_before_multitask_sequential_training(self, sentence_tensor: torch.Tensor, lengths: torch.LongTensor, sentences=None, prior_mode='global'):  # type: ignore[override]
        """
        Forward propagation through network.
        :param sentence_tensor: A tensor representing the batch of sentences.
        :param lengths: A IntTensor representing the lengths of the respective sentences.
    ...
    def _calculate_loss(self, scores: torch.Tensor, labels: torch.LongTensor) -> Tuple[torch.Tensor, int]:
        ...
        if self.unc_args.use_uce == 3:
            return self.loss_function(scores, labels, self.unc_args.output_dim), len(labels)
        return self.loss_function(scores, labels), len(labels)   # F.cross_entropy loss
    ...
    ### self-added for t-SNE
    def get_latent_feature(self, batch):

        sentence_tensor, lengths = self._prepare_tensors(batch)

        # below is part of Func: self.predict
        if self.use_dropout:
            sentence_tensor = self.dropout(sentence_tensor)
        if self.use_word_dropout:
            sentence_tensor = self.word_dropout(sentence_tensor)
        ...
        # cal culate token-level latent embedding (from uncertainty view using density-based method)
        if self.unc_args.unc_method in ['slpn']:
            latent_tensor = self.token_level_pn.latent_encoder(sentence_tensor)

        return sentence_tensor, latent_tensor
    ...
    def predict(
        ...
        with torch.no_grad():
            ...
            for batch in dataloader:
                ...
                sentence_tensor, lengths = self._prepare_tensors(batch)
                # features = self.forward(sentence_tensor, lengths) # this is original version

                ## below is original slpn part before adding sequential training
                if self.unc_args.unc_method is None or self.unc_args.prior_mode == 'global':
                    features = self.forward(sentence_tensor, lengths)
                    features, latent_tensor = self.forward(sentence_tensor, lengths)
                elif self.unc_args.unc_method is not None and 'local' in self.unc_args.prior_mode:

                    features, latent_tensor = self.forward(sentence_tensor, lengths, sentences, self.unc_args.prior_mode)  # features = alpha = 1 + beta

                if (self.unc_args.use_seq_training and not self.unc_args.finished_pretr_process) or (self.unc_args.use_multitask_training):
                    latent_scores = self.latent2label_linear(latent_tensor)
                    latent_scores = latent_scores.squeeze(0)
                    features = latent_scores
                    # print(f"latent_scores has featues of {latent_scores.shape}")
                ...
                if return_loss:
                    ...
                    if (self.unc_args.use_seq_training and not self.unc_args.finished_pretr_process):
                        loss = torch.nn.CrossEntropyLoss(weight=self.loss_weights, reduction="sum")(features, gold_labels), len(gold_labels)
                    elif self.unc_args.use_multitask_training:
                        loss = self._calculate_loss(features, gold_labels)[0] + self.unc_args.bert_loss_w * torch.nn.CrossEntropyLoss(weight=self.loss_weights, reduction="sum")(features, gold_labels), len(gold_labels)
                    else:
                ...
                # make predictions
                if self.use_crf:
                    predictions, all_tags, all_softmax = self.viterbi_decoder.decode(
                        features, return_probabilities_for_all_classes, batch
                    )  # all_softmax might not yet implemented
                else:
                    predictions, all_tags, all_softmax = self._standard_inference(
                        features, batch, return_probabilities_for_all_classes, use_alpha, sfmx_mode
                    )
                ...
                for sentence, sentence_predictions in zip(batch, predictions):
                    ...
                    # BIOES-labels need to be converted to spans # span-level labels
                    if self.predict_spans and not force_token_predictions:
                        ...
                        predicted_spans = get_spans_from_bio(sentence_tags, sentence_scores) # merge token-level labels to span-level labels
                        if len(predicted_spans) != 0:
                            # print(predicted_spans)
                            pass
                        for predicted_span in predicted_spans:
                            span: Span = sentence[predicted_span[0][0] : predicted_span[0][-1] + 1]
                            span.add_label(label_name, value=predicted_span[2], score=predicted_span[1]) # self-comment: need to consider change score into the vectors
                ...
                for (sentence, sent_all_tags) in zip(batch, all_tags):
                    for (token, token_all_tags) in zip(sentence.tokens, sent_all_tags):
                        token.add_tags_proba_dist(label_name, token_all_tags)    # saved_attri: tags_proba_dist; sentence.tokens[0].tags_proba_dist['predicted'][1].value
            ...
            if return_loss:
                return overall_loss, label_count, all_softmax, self.label_dictionary

    def _standard_inference(self, features: torch.Tensor, batch: List[Sentence], probabilities_for_all_classes: bool, use_alpha: bool, sfmx_mode: str):
        """
        ...
        if self.unc_args.unc_method is None:
            if sfmx_mode == 'sfmx':
                softmax_batch = F.softmax(features,
                                          dim=1).cpu()  # original, since we have softmax in model.py -> def map_dict2vec
            elif sfmx_mode == 'mean':
                softmax_batch = (features / features.sum(dim=-1).unsqueeze(
                    -1)).cpu()  # original, since we have softmax in model.py -> def map_dict2vec
            elif sfmx_mode is None:
                softmax_batch = F.softmax(features,
                                          dim=1).cpu()  # original, since we have softmax in model.py -> def map_dict2vec
                # softmax_batch = features.cpu()  # new for alpha usage, since we have softmax in model.py -> def map_dict2vec
            else:
                raise ValueError(f'sfmx_mode={sfmx_mode} is set wrongly')
            # softmax_batch = F.softmax(features, dim=1).cpu() # original, since we have softmax in model.py -> def map_dict2vec
        elif self.unc_args.unc_method in ['slpn']:
            if self.unc_args.use_seq_training==True and not self.unc_args.finished_pretr_process:
                # in the partial training process
                softmax_batch = F.softmax(features, dim=1).cpu()
            else:
                if use_alpha == False:
                    if sfmx_mode == 'sfmx':
                        softmax_batch = F.softmax(features, dim=1).cpu() # which could be wrong for alpha since we have softmax in model.py -> def map_dict2vec
                    elif sfmx_mode == 'mean':
                        softmax_batch = (features / features.sum(dim=-1).unsqueeze(-1)).cpu()  # which should be correct for alpha, since we have softmax in model.py -> def map_dict2vec
                    elif sfmx_mode is None:
                        softmax_batch = (features / features.sum(dim=-1).unsqueeze(-1)).cpu()
                        # softmax_batch = features.cpu()  # new for alpha usage, since we have softmax in model.py -> def map_dict2vec
                    else:
                        raise ValueError(f'sfmx_mode={sfmx_mode} is set wrongly')
                else:
                    if sfmx_mode is None:
                        softmax_batch = features.cpu()  # new for alpha usage, since we have softmax in model.py -> def map_dict2vec
                    else:
                        raise ValueError(f'sfmx_mode=None is set wrongly')
        ...
        for sentence in batch:
            ...
            scores_batch = scores_batch[len(sentence) :] # me: the sentences have been concatenated, this operation removes the sentence that has been processed
        ...
        # self-added:
        softmax_batch2 = F.softmax(features, dim=1).cpu()
        all_softmax = []
        for sentence in batch:
            one_sen_softmax = softmax_batch2[: len(sentence)]

            all_softmax.append(one_sen_softmax)
            softmax_batch2 = softmax_batch2[len(sentence) :]
        ...
        return predictions, all_tags, all_softmax
    ...
    def _all_scores_for_token(self, sentences: List[Sentence], scores: torch.Tensor, lengths: List[int]):
        ...
        for length in lengths:
            ...
            # previous = length
            previous = previous + length # self-revise
...
from flair.models import SequenceTaggerWiUnc

# load tagger
tagger = SequenceTaggerWiUnc.load("{repo_id}")
...
```"""
    ...
    def push_to_hub(
        ...
        private: bool = None,
        commit_message: str = "Add new SequenceTaggerWiUnc model.",
    ):
        """
    ...
    # below is designed for SLPN
    def extract_cat_distribution(self, corpus):
        # refer to flair/trainers/trainer.py
        train_data = corpus.train
        batch_loader = DataLoader(
            train_data,
            batch_size=1, # should be fixed as 1 in current implemention
        ...
        gl_cat_list = [0] * self.label_dictionary.__len__()
        token_cat_list_dict = {}
        ...
        for batch_no, batch in enumerate(batch_loader):
            ...
            for batch_step in batch_steps:
                ...
                sentences = sorted(sentences, key=len, reverse=True)
                gold_labels = self._prepare_label_tensor(sentences)
                gold_labels_list = gold_labels.tolist()
                token_list = sentences[0].text.split(' ')
                for i in range(len(gold_labels_list)):
                    gl_cat_list[gold_labels_list[i]] += 1
                    if token_list[i] not in token_cat_list_dict.keys():
                        token_cat_list_dict[token_list[i]] = [0] * self.label_dictionary.__len__()
                    else:
                        token_cat_list_dict[token_list[i]][gold_labels_list[i]] += 1

        gl_cat_dict = {}
        for j in range(len(gl_cat_list)):
            item_name = self.label_dictionary.get_item_for_index(j)
            gl_cat_dict[item_name] = gl_cat_list[j]

        # tranfer abs_vals to distribution
        gl_cat_dis_list = list(np.array(gl_cat_list) / np.array(gl_cat_list).sum())
        token_cat_dis_list_dict = {}
        for key in token_cat_list_dict.keys():
            token_cat_dis_list_dict[key] = list(np.array(token_cat_list_dict[key]) / (np.array(token_cat_list_dict[key]).sum()+1))


        return gl_cat_list, gl_cat_dict, gl_cat_dis_list, token_cat_list_dict, token_cat_dis_list_dict
...
class MultiTagger:
    def __init__(self, name_to_tagger: Dict[str, SequenceTaggerWiUnc]):
        super().__init__()
    ...
    def load(cls, model_names: Union[List[str], str]):
        ...
        taggers = {}
        models: List[SequenceTaggerWiUnc] = []

        # load each model
        for model_name in model_names:

            model = SequenceTaggerWiUnc.load(model_name)
